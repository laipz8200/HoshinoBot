...
class TLSqliteDao:
    ...
    def _add_approval(self, tid, approval):
        try:
            conn = self._connect()
            r = conn.execute(
                "SELECT APPROVAL FROM TLTABLE WHERE ID=?", (tid,)).fetchone()
            a = r[0]
            conn.execute(
                "UPDATE TLTABLE SET APPROVAL=? WHERE ID=?", (a+approval, tid))
            conn.commit()
        except:
    ...
    def _del_by_id(self, tid, userid):
        try:
            conn = self._connect()
            r = conn.execute(
                "SELECT USERID FROM TLTABLE WHERE ID=?", (tid,)).fetchone()
    ...
    def _update_by_id(self, tid, damage, introduction, timeline, userid):
        try:
            ...
            if userid != r[0] and userid != 999:
                ...
                conn.execute("UPDATE TLTABLE SET DAMAGE=?, INTRODUCTION=?, TIMELINE=? WHERE ID=?;",
                             (damage, introduction, timeline, tid))
                conn.commit()
                return True
        except:
    ...
    def _find_by_id(self, tid):
        try:
            r = self._connect().execute(
                "SELECT BOSSNAME,DAMAGE,TIMELINE,INTRODUCTION FROM TLTABLE WHERE ID=?", (tid,)).fetchone()
            return r
        except:
    ...
    def _find_by_bossname(self, bossname):
        try:
            r = self._connect().execute(
                "SELECT ID,BOSSNAME,DAMAGE,INTRODUCTION,APPROVAL FROM TLTABLE WHERE BOSSNAME LIKE '%s%%' ORDER BY APPROVAL DESC" % bossname).fetchall()
    ...
    def _find_all(self):
        try:
            r = self._connect().execute(
                "SELECT ID,BOSSNAME,DAMAGE,INTRODUCTION,APPROVAL FROM TLTABLE ORDER BY APPROVAL DESC").fetchall()
            return self._2dtuple_to_1dlist(r)
        except:
            raise Exception('查找轴发生错误')
