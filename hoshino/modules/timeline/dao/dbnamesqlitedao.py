...
DB_PATH = os.path.expanduser('~/.hoshino/timeline/timeline_database_name.db')
...
class TLDBNameDao:
    ...
    def _update_by_id(self, group_id, tldb_name):
        try:
            conn = self._connect()
            r = conn.execute(
                "SELECT * FROM TLDBNAMETABLE WHERE GID=?", (group_id,)).fetchone()
            if r != None:
                conn.execute(
                    "UPDATE TLDBNAMETABLE SET TLDBNAME=? WHERE GID=?", (tldb_name, group_id))
                conn.commit()
            else:
    ...
    def _find_by_id(self, group_id):
        try:
            r = self._connect().execute(
                "SELECT TLDBNAME FROM TLDBNAMETABLE WHERE GID=?", (group_id,)).fetchone()
            return r[0]
        except:
