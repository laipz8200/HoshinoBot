...
async def jc(bot, ev: CQEvent):
    ...
    font = ImageFont.truetype(os.path.join(
        os.path.dirname(__file__), 'simhei.ttf'), 80)
...
async def friend(bot, ev: CQEvent):
    ...
    data = load_config(os.path.join(
        os.path.dirname(__file__), 'config.json'))['friend']
    arr = []
    is_at = False
    for m in ev.message:
    ...
    # 创建Font对象:
    font = ImageFont.truetype(os.path.join(
        os.path.dirname(__file__), 'simhei.ttf'), 30)
    font2 = ImageFont.truetype(os.path.join(
        os.path.dirname(__file__), 'simhei.ttf'), 25)
...
async def diary(bot, ev: CQEvent):
    ...
    font = ImageFont.truetype(os.path.join(
        os.path.dirname(__file__), 'simhei.ttf'), font_size)
    ...
    # 模糊:
    # image_text = image_text.filter(ImageFilter.BLUR)
    image_back = Image.new(
        'RGB', (img_width, line_h * line + img_height), (255, 255, 255))
    image_back.paste(image, (0, 0))
